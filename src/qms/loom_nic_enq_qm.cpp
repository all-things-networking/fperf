...
void LoomNICEnqQM::constrs_if_not_taken(NetContext& net_ctx,
    ...
    for (unsigned int t = 0; t < total_time; t++){
        ...
        vector<expr> le_i_gets_j(per_queue_share * in_queue_cnt,
        ...
        for (unsigned int i = 0; i < per_queue_share * in_queue_cnt; i++){
            ...
            for (unsigned int j = i; j < per_queue_share * in_queue_cnt; j++){
                ...
                sprintf(constr_name,
                        ...
                        id.c_str(), i, src_qid, src_elem_id, t);
        ...
        le_i_gets_j = vector<expr>(per_queue_share * in_queue_cnt,
                                  net_ctx.bool_val(false));
        for (unsigned int i = 0; i < per_queue_share * in_queue_cnt; i++){
        ...
        for (unsigned int q = 0; q < out_queues.size(); q++){
            ...
            for (unsigned int i = per_queue_share * in_queue_cnt;
                 i < outq->max_enq(); i++){
...
void LoomNICEnqQM::add_constrs(NetContext& net_ctx,
                               std::map<std::string, expr>& constr_map){
