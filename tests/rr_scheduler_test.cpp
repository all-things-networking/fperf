...
bool test_unsat_query() {
  ...
  for (unsigned int q = 0; q < in_queue_cnt; q++) {
      auto comp_spec1 = std::make_shared<Comp>(Indiv(metric_t::CENQ, q), op_t::GE, Time(1));
      wl.add_spec(TimedSpec(comp_spec1, time_range_t(0, total_time - 1), total_time));
  }

  auto comp_spec2 = std::make_shared<Comp>(Indiv(metric_t::CENQ, queue1), op_t::GT, Indiv(metric_t::CENQ, queue2));
...
bool test_sat_query() {
  ...
  for (unsigned int i = 1; i <= recur; i++) {
      for (unsigned int q = 0; q < in_queue_cnt; q++) {
          auto comp_spec1 = std::make_shared<Comp>(Indiv(metric_t::CENQ, q), op_t::GE, i * rate);
