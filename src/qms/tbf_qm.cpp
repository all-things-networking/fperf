...
void TBFQM::add_constrs(NetContext& net_ctx, std::map<std::string, expr>& constr_map) {
    ...
    for (unsigned int t = 1; t < total_time; t++) {
        for (unsigned int i = 0; i <= in_queue->size(); i++) {
            ...
            if (i > 0)
                is_last_null_packet = is_last_null_packet &&
    ...
    for (unsigned int t = 0; t < total_time; t++) {
        ...
        if (t == 0)
            ...
            constr_expr = token_queue[t] == max(net_ctx.int_val(0),
                                                min(net_ctx.int_val(info.max_tokens),
