...
TSUM::TSUM(qset_t qset, metric_t metric):
qset(qset),
metric(metric)
{}
...
TONE::TONE(metric_t metric, unsigned int queue):
metric(metric),
queue(queue)
{}
...
bool operator== (const trf_t& trf1, const trf_t& trf2){
    if (std::holds_alternative<TSUM>(trf1) &&
        std::holds_alternative<TSUM>(trf2)){
        return std::get<TSUM>(trf1) == std::get<TSUM>(trf2);
    }
    if (std::holds_alternative<TONE>(trf1) &&
        std::holds_alternative<TONE>(trf2)){
...
bool operator==(const rhs_t& rhs1, const rhs_t& rhs2){
    if (std::holds_alternative<trf_t>(rhs1) &&
        std::holds_alternative<trf_t>(rhs2)){
...
Unique::Unique(qset_t qset, metric_t metric):
qset(qset),
metric(metric)
{}
...
WlSpec::WlSpec(lhs_t lhs, comp_t comp, rhs_t rhs):
lhs(lhs),
comp(comp),
rhs(rhs)
{
    ...
    if (holds_alternative<trf_t>(rhs)){
        ...
        if (lhs_is_tsum && rhs_is_tsum){
            ...
            if (lhs_tsum.get_metric() == rhs_tsum.get_metric()){
                ...
                if (lhs_qset == rhs_qset){
                    if (comp == comp_t::GE ||
                        comp == comp_t::LE) is_all = true;
                    if (comp == comp_t::GT ||
                        comp == comp_t::LT) is_empty = true;
                }
                    ...
                    if (comp == comp_t::LE) is_all = true;
                    else if (comp == comp_t::GT) is_empty = true;
                    else if (diff.size() == 1){
                        lhs = TONE(rhs_tsum.get_metric(), *(diff.begin()));
                        if (comp == comp_t::EQ) comp = comp_t::LE;
                        else comp = neg_comp(comp);
                        rhs = 0u;
                    }
                    else {
                    ...
                    if (comp == comp_t::LT) is_empty = true;
                    else if (comp == comp_t::GE) is_all = true;
                    else if (diff.size() == 1){
                        lhs = TONE(rhs_tsum.get_metric(), *(diff.begin()));
                        rhs = 0u;
                    }
                    ...
                    if (lhs_diff.size() == 1){
                        ...
                        lhs = TSUM(qset_t(lhs_diff.begin(), lhs_diff.end()),
                                   lhs_tsum.get_metric());
                    }
                    ...
                    if (rhs_diff.size() == 1){
                        ...
                        rhs = TSUM(qset_t(rhs_diff.begin(), rhs_diff.end()),
                                   rhs_tsum.get_metric());
                    }
                }
            }
        ...
        // if both are TONE
            ...
            if (lhs_tone == rhs_tone){
                if (comp == comp_t::LE ||
                    comp == comp_t::GE) is_all = true;
                else if (comp == comp_t::LT ||
                         comp == comp_t::GT) is_empty = true;
            }
        ...
        if (lhs_is_tsum && rhs_is_tone){
            ...
            if (lhs_tsum.get_metric() == rhs_tone.get_metric()){
                ...
                if (it != lhs_qset.end()){
                    if (comp == comp_t::GE) is_all = true;
                    else if (comp == comp_t::LT) is_empty = true;
                    else {
                        lhs_qset.erase(it);
                        rhs = 0u;
                        if (lhs_qset.size() == 1){
    ...
    if (holds_alternative<TIME>(rhs)){
        ...
        if (c == 0){
            ...
            if (holds_alternative<TSUM>(lhs)){
                ...
                if (comp == comp_t::GE &&
                    c > (tsum.get_qset().size() - 1) * MAX_ENQ)
                    is_empty = true;
                if (comp == comp_t::EQ &&
                    c > (tsum.get_qset().size() - 1) * MAX_ENQ)
                    is_empty = true;
...
std::ostream& operator<<(std::ostream& os, const WlSpec& spec){
    if (spec.is_all) os << "*";
    else if (spec.is_empty) os << "FALSE";
    else os << spec.lhs << " " << spec.comp << " " << spec.rhs;
    return os;
}
...
void TimedSpec::normalize(){
    if (wl_spec.spec_is_empty()) is_empty = true;
    else if (wl_spec.spec_is_all() ||
...
void Workload::normalize(time_range_t time_range){
    ...
    for (set<WlSpec>::iterator it = specs.begin();
        ...
        else non_zero_specs.push_back(spec);
    }
...
void Workload::regenerate_spec_set(){
    ...
    for (timeline_t::iterator it = timeline.begin();
        ...
        for (set<WlSpec>::iterator s_it = specs.begin();
             s_it != specs.end(); s_it++){
...
string Workload::get_timeline_str(){
    ...
    if (is_empty()){
        ...
        for (timeline_t::iterator it = timeline.begin();
            ...
            bool is_first = true;
            for (set<WlSpec>::iterator it2 = specs.begin();
                 it2 != specs.end(); it2++){
                if (!is_first){
                    ss << "        ";
                }
