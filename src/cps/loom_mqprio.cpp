...
void LoomMQPrio::add_nodes(){
    ...
    for (unsigned int i = 0; i < nic_tx_queue_cnt; i++){
        ...
        LoomFlowEnqQM* flow_enq_qm = new LoomFlowEnqQM(m_id,
                                                       total_time,
                                                       vector<QueueInfo>(per_core_flow_cnt, info),
                                                       info,
                                                       info,
                                                       net_ctx);
        ...
        RRQM* rr_qm = new RRQM(m_id,
                               total_time,
                               vector<QueueInfo>(tenant_cnt, info),
                               info,
                               net_ctx);
    ...
    cid_t m_id = "NICEnq";
    LoomNICEnqQM* nic_enq_qm = new LoomNICEnqQM(m_id,
                                                total_time,
                                                vector<QueueInfo>(nic_tx_queue_cnt, info),
                                                info,
                                                info,
                                                net_ctx);
    nodes.push_back(m_id);
    ...
    m_id = "EndDemux";
    LoomDemuxQM* demux_qm = new LoomDemuxQM(m_id,
                                            total_time,
                                            info,
                                            info, info,
                                            net_ctx);
...
std::string LoomMQPrio::cp_model_str(model& m,
                                     NetContext& net_ctx,
                                     unsigned int t){
    // TODO: implement
    (void) net_ctx;
