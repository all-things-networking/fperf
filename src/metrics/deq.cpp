...
unsigned int Deq::eval(const IndexedExample* eg, unsigned int time, unsigned int qind) {
    return eg->deqs[qind][time];
}

void Deq::add_vars(NetContext& net_ctx) {
    (void) net_ctx;
}

void Deq::add_constrs(NetContext& net_ctx, std::map<std::string, expr>& constr_map) {
    (void) net_ctx;
