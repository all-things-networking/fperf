...
CEnq::CEnq(Queue* queue, unsigned int total_time, NetContext& net_ctx):
Metric(metric_t::CENQ, queue, total_time, net_ctx) {
    init(net_ctx);
}

unsigned int CEnq::eval(const IndexedExample* eg, unsigned int time, unsigned int qind) {
    unsigned int res = 0;
...
void CEnq::add_constrs(NetContext& net_ctx, std::map<std::string, expr>& constr_map) {
