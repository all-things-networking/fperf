...
void ContentionPoint::add_same_to_base(Same same, time_range_t time_range){
    ...
    /*
    if (metric == metric_t::META1){
        std::snprintf(constr_name, 100, "%s_enqs_0_have_same_meta1_%d", id.c_str(), t);
    }
    else {
        std::snprintf(constr_name, 100, "%s_enqs_0_have_same_meta2_%d", id.c_str(), t);
    }
    */
...
expr ContentionPoint::get_expr(Query& query){
    ...
    for (unsigned int t = time_range.first; t <= time_range.second; t++){
        ...
        query_vec.push_back(mk_and(valid) && 
                            mk_op(lhs,
                                  query.get_op(),
                                  rhs));
    }
...
expr ContentionPoint::get_expr(Comp comp, unsigned int t){
    ...
    return (lhs.first && rhs.first && 
            mk_op(lhs.second, comp.get_op(), rhs.second));
}
...
m_val_expr_t ContentionPoint::get_expr(Time time, unsigned int t){
    return m_val_expr_t(net_ctx.bool_val(true), 
                        net_ctx.int_val(time.get_coeff() * (t + 1)));
}
...
bool ContentionPoint::eval_spec(Incr incr,
                                IndexedExample* eg,
                                time_range_t time_range) const {
...
bool ContentionPoint::eval_spec(Decr decr,
                                IndexedExample* eg,
                                time_range_t time_range) const {
...
bool ContentionPoint::eval_spec(Comp comp,
                                IndexedExample* eg,
                                time_range_t time_range) const {
    for (unsigned int t = time_range.first;
         t <= time_range.second; t++){
...
void ContentionPoint::eval_m_expr(m_expr_t m_expr,
                                  ...
                                  unsigned int time,
                                  metric_val& res) const {
    visit([&eg, time, res, this](auto const& sf){this->eval_m_expr(sf, eg, time, res);}, m_expr);
}
