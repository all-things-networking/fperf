...
void run(ContentionPoint* cp,
         ...
         unsigned int good_example_cnt, std::string good_examples_file,
         unsigned int bad_example_cnt, std::string bad_examples_file,
         Query& query,
...
void loom(std::string good_examples_file,
    ...
    // Create contention point
    LoomMQPrio* cp = new LoomMQPrio(nic_tx_queue_cnt,
                                    per_core_flow_cnt,
                                    total_time);
    ...
    for (unsigned int i = 0; i < cp->in_queue_cnt(); i++){
        if (i % 3 == 0) tenant1_qset.insert(i);
        else tenant2_qset.insert(i);
    }
...
void leaf_spine_bw(std::string good_examples_file,
    ...
    // Create contention point
    LeafSpine* cp = new LeafSpine(leaf_cnt, spine_cnt,
                                  servers_per_leaf,
                                  total_time,
                                  reduce_queues);
...
void run(ContentionPoint* cp,
         IndexedExample* base_eg,
         unsigned int good_example_cnt, string good_examples_file,
         unsigned int bad_example_cnt, string bad_examples_file,
         Query& query,
         unsigned int max_spec,
         SharedConfig* config){
    ...
    cout << "good example generation: " << (get_diff_millisec(start_time, noww())/1000.0) << " s" << endl;
