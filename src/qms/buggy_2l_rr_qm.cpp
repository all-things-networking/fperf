...
QueuingModule(id, total_time, in_queue_info, vector<QueueInfo>{out_queue_info}, net_ctx) {
    ...
    old_queues = new Queue(id, "old_queues", in_queues.size() + 2, 1, 1, total_time, net_ctx);
...
void Buggy2LRRQM::add_constrs(NetContext& net_ctx, map<string, expr>& constr_map) {
    ...
    for (unsigned int t = 0; t < total_time; t++) {
        ...
        for (unsigned int q = 0; q < in_queue_cnt; q++) {
            ...
            prev_unavailable.push_back(prev_unavailable[q] &&
                                       net_ctx.pkt2val(new_queues->enqs(q)[t]));

            activated_vec.push_back(
                ite(inactive_[q][t] && has_enq, net_ctx.int_val(1), net_ctx.int_val(0)));
        }
        ...
        for (unsigned int q = 0; q < in_queue_cnt; q++) {
            ...
            expr per_queue_expr = implies(dequeued_from_q && not_empty_next_round, insert_into_old);
        ...
        expr no_old_enq1 = !net_ctx.pkt2val(new_queues_head) && !net_ctx.pkt2val(old_queues_head);

        expr no_old_enq2 = (net_ctx.pkt2val(new_queues_head) && mk_or(new_head_queue_empty_vec));

        expr no_old_enq3 = (!net_ctx.pkt2val(new_queues_head) && net_ctx.pkt2val(old_queues_head) &&
