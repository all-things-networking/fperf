...
void SpineForwardingQM::add_constrs(NetContext& net_ctx,
    ...
    for (unsigned int t = 0; t < total_time; t++){
        ...
        for (unsigned int dst = 0; dst < server_cnt; dst++){
            ...
            sprintf(constr_name, "%s_forward_dst_%d_to_port%d(q%d)_at_%d", id.c_str(), dst, dst_port, voq_ind, t);
        ...
        for (unsigned int i = 0; i < leaf_cnt; i++){
            ...
            constr_expr = implies(pkt_dst < (int) (i * servers_per_leaf) || pkt_dst >= (int) ((i + 1) * servers_per_leaf),
        ...
        for (unsigned int q = 0; q < in_queues.size(); q++){
            ...
            for (unsigned int p = 0; p < queue->size(); p++){
                ...
                sprintf(constr_name, "%s_flow_meta_bounds_input_queue[%d][%d][%d]", id.c_str(), q, p, t);
        ...
        for (unsigned int q = 0; q < out_queues.size(); q++){
            ...
            for (unsigned int p = 0; p < queue->size(); p++){
                ...
                sprintf(constr_name, "%s_flow_meta_bounds_output_queue[%d][%d][%d]", id.c_str(), q, p, t);
                constr_expr = implies(pkt_val, pkt_meta == (int) spine_id);
                constr_map.insert(named_constr(constr_name, constr_expr));
            }
        }
