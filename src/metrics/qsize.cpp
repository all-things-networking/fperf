...
unsigned int QSize::eval(const IndexedExample* eg,
                        unsigned int time,
                        unsigned int qind){
...
void QSize::add_constrs(NetContext& net_ctx,
    ...
    for (unsigned int t = 0; t < total_time; t++){
        ...
        constr_expr = implies(net_ctx.pkt2val(queue->elem(last_elem)[t]), val_[t] == (int)(last_elem + 1));
        constr_map.insert(named_constr(constr_name, constr_expr)); 
    } 
    /*
